...
const ENV_PREFIX = "SB_";
/** The catalogue keys carry a prefix of their own, declared in the registry. */
const KEY_PREFIX = "STASHBOX_";

const ROOT = join(import.meta.dirname, "..", "..");
const readme = readFileSync(join(ROOT, "README.md"), "utf8");
const configSource = [
  readFileSync(join(ROOT, "src", "config.ts"), "utf8"),
  readFileSync(join(ROOT, "src", "stashbox", "instances.ts"), "utf8"),
].join("\n");
...
describe("the settings", () => {
  it("are the ones the code reads, and all of them", () => {
    const pattern = new RegExp(`\\b((?:${ENV_PREFIX}|${KEY_PREFIX})[A-Z_]+)\\b`, "g");
